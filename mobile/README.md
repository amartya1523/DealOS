...
| `DEALOS_GOOGLE_IOS_CLIENT_ID` | Optional override for the iOS OAuth client configured in `Info.plist`; the backend's web client ID remains the token audience |
...
The backend must set `GOOGLE_CLIENT_ID` to its web OAuth client ID and `GOOGLE_IOS_CLIENT_ID` to the trusted iOS client ID so it can verify either registered token audience. Android also needs OAuth registrations for the applicable package names and signing certificate fingerprints; configure its trusted audience as `GOOGLE_ANDROID_CLIENT_ID`. The Development iOS client and its reversed callback scheme are registered in `ios/Runner/Info.plist`; production distribution requires a separate client for `com.dealos.mobile` before release. `DEALOS_GOOGLE_IOS_CLIENT_ID` can override the plist client in CI, but its reversed scheme must also be registered in the built app.
