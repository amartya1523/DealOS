...
async function createInvoice(input: {
  ...
  if (input.state === 'PARTIAL' || input.state === 'PAID') {
    await db.payment.create({ data: { invoiceId: invoice.id, amount: paidAmount, reference: `UTR-${invoice.number}-${input.state}`, paidAt: atDay(-2, 12), createdAt: atDay(-2, 12) } });
  }
  if (input.state === 'REVERSED') {
    const original = await db.payment.create({ data: { invoiceId: invoice.id, amount: reversedAmount, reference: `UTR-${invoice.number}-ORIGINAL`, paidAt: atDay(-4, 12), createdAt: atDay(-4, 12) } });
    await db.payment.create({ data: { invoiceId: invoice.id, amount: reversedAmount, reference: `REV-${invoice.number}`, paidAt: atDay(-3, 12), reversalOfId: original.id, reason: 'Duplicate bank posting corrected in the seed scenario.', createdAt: atDay(-3, 12) } });
  }
