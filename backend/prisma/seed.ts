...
async function main() {
  ...
  const [acme, beta, northstar] = await Promise.all([
    db.customer.create({ data: { name: 'Acme Corp', tier: 'Gold', currency: 'USD' } }),
    db.customer.create({ data: { name: 'Beta Industries', tier: 'Silver', currency: 'USD' } }),
    db.customer.create({ data: { name: 'Northstar Labs', tier: 'Bronze', currency: 'USD' } }),
  ]);
  ...
  ].map(([name, email, role]) => db.user.create({ data: { name: String(name), email: String(email), role: role as Role, customerId: role === Role.CUSTOMER ? acme.id : null, passwordHash, status: 'ACTIVE' } })));
  const rep = users[0]!;
  ...
  const q102 = await db.quote.create({ data: { number: 'Q-0102', customer: acme.name, customerId: acme.id, customerTier: acme.tier, ownerId: rep.id, stage: 'PENDING_APPROVAL', total: q102Calc.total, taxTotal: q102Calc.taxTotal, totalsByCadence: json(q102Calc.totalsByCadence), margin: q102Calc.margin, riskScore: q102Calc.riskScore, lines: { create: q102Lines.map((line) => ({ productId: line.product.id, quantity: line.quantity, unitPrice: line.product.price, unitCost: line.product.cost, discount: line.discount, allowedDiscount: line.allowedDiscount })) } } });
  ...
  for (const [number, customer, stage] of [['Q-0103', beta, 'DRAFT'], ['Q-0104', northstar, 'DRAFT']] as const) {
    const quote = await db.quote.create({ data: { number, customer: customer.name, customerId: customer.id, customerTier: customer.tier, ownerId: rep.id, stage } });
  ...
  await db.alert.createMany({ data: [
    ...
    { kind: 'DISCOUNT_ANOMALY', title: 'Discount is 2.4× the rep baseline', detail: 'Q-0102 · Setup service discount is 18%', severity: 'High', resourceId: q102.id },
