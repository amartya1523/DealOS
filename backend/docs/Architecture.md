...
Do not maintain authoritative totals in browser storage. Query caching is ephemeral. Database migrations are committed and applied separately from API startup. Twenty-five migrations currently implement the functional subset described in [Database.md](Database.md#current-database-state); the larger baseline remains a target contract.
