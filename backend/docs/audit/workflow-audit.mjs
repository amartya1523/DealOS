...
try{
 ...
 const customers=new Map([[acme.name,acme]]);
 async function fixtureCustomer(name){
  const existing=customers.get(name);
  if(existing)return existing;
  const created=await db.customer.create({data:{organizationId,name,tier:'Gold',currency:'INR'}});
  customers.set(name,created);
  return created;
 }
 async function make(customer='Acme Audit',discount=18,p=service,owner='REP'){
  const customerRecord=await fixtureCustomer(customer);
  const c=await api(owner,'/quotations',{customerId:customerRecord.id});
  if(c.status!==201)throw new Error(`Fixture quote failed (${c.status} ${c.error?.code??'UNKNOWN'}: ${c.error?.message??'no message'})`);
  ...
  if(s.status!==200)throw new Error(`Fixture draft failed (${s.status} ${s.error?.code??'UNKNOWN'}: ${s.error?.message??'no message'})`);
  return s.data.quote;
 }
 async function submit(q,role='REP'){
  const response=await api(role,`/quotations/${q.id}/submit`,{});
  if(response.status!==200)throw new Error(`Fixture submit failed (${response.status} ${response.error?.code??'UNKNOWN'}: ${response.error?.message??'no message'})`);
  return db.approval.findMany({where:{quoteId:q.id},orderBy:[{cycle:'asc'},{sequence:'asc'}]});
 }
 async function approveAndSend(q,owner='REP'){
  const steps=await submit(q,owner);
  for(const step of steps){
   const reviewer=step.step==='Finance'?'FINANCE':'MANAGER';
   const decision=await api(reviewer,`/approvals/${step.id}/decision`,{decision:'APPROVE',reason:'Audit workflow approval'});
   if(decision.status!==200)throw new Error(`Fixture approval failed (${decision.status} ${decision.error?.code??'UNKNOWN'}: ${decision.error?.message??'no message'})`);
  }
  const sent=await api(owner,`/quotations/${q.id}/send`,{});
  if(sent.status!==200)throw new Error(`Fixture send failed (${sent.status} ${sent.error?.code??'UNKNOWN'}: ${sent.error?.message??'no message'})`);
  return sent.data;
 }
 ...
 const other=await make('Other Company');await approveAndSend(other);
 ...
 const stock=await make('Acme Audit',0,product);await approveAndSend(stock);r=await api('CUSTOMER',`/portal/quotations/${stock.id}/confirm`,{});record('Approved customer quote can confirm',r.status===200,r.status);
 ...
 const shortage=await make('Acme Audit',0,product);const shortageDraft=await api('REP',`/quotations/${shortage.id}/draft`,{version:shortage.version,orderDiscount:0,lines:[{productId:product.id,quantity:100,discount:0}]},'PUT');await approveAndSend(shortageDraft.data.quote);await api('CUSTOMER',`/portal/quotations/${shortage.id}/confirm`,{});const shortagePreview=await api('FINANCE',`/fulfillment/${shortage.id}/preview`);r=await api('FINANCE',`/fulfillment/${shortage.id}/allocate`,{stockFingerprint:shortagePreview.data.stockFingerprint});record('Partial allocation creates explicit backorder',r.data?.state==='BACKORDER'&&r.data?.split?.backorders?.[0]?.quantity===2,{state:r.data?.state,split:r.data?.split});
 ...
 const dup=await make('Acme Audit',0,product);const dupSaved=await api('REP',`/quotations/${dup.id}/draft`,{version:dup.version,orderDiscount:0,lines:[{productId:product.id,quantity:60,discount:0},{productId:product.id,quantity:60,discount:0}]},'PUT');await approveAndSend(dupSaved.data.quote);await api('CUSTOMER',`/portal/quotations/${dup.id}/confirm`,{});await api('FINANCE',`/fulfillment/${dup.id}/allocate`,{});const over=await db.stockBalance.findFirst({where:{productId:product.id}});record('Duplicate SKU lines cannot overreserve stock',over.reserved<=over.onHand,{onHand:over.onHand,reserved:over.reserved});
...
if(failures.length){
 console.error(`Audit failed: ${failures.length} of ${results.length} checks failed.`);
 process.exitCode=1;
}else console.log(`Audit passed: ${results.length} checks passed.`);
