...
try{
 ...
 const acme=await db.customer.create({data:{name:'Acme Audit',tier:'Gold'}});
 const users={};
 for(const role of ['REP','MANAGER','FINANCE','ADMIN','CUSTOMER']) users[role]=await db.user.create({data:{name:`Audit ${role}`,email:`${role.toLowerCase()}@audit.invalid`,passwordHash:hash,role,status:'ACTIVE',customerId:role==='CUSTOMER'?acme.id:null}});
 for(const role of Object.keys(users)){const r=await api(role,'/auth/login',{email:users[role].email,password:'AuditOnly2026!'});sessions[role]=r.cookie?.split(';')[0];csrf[role]=r.data?.csrfToken;record(`${role} login`,r.status===200,r.status)}
 ...
 await api('ADMIN',`/policies/${policy.id}`,{maxDiscount:15,financeThreshold:5},'PATCH');
 ...
 const inv=await db.invoice.create({data:{number:'AUD-INV',quoteId:stock.id,orderId:(await db.order.findUnique({where:{quoteId:stock.id}})).id,customer:'Acme Audit',customerId:acme.id,amount:1000,dueAt:new Date(),lines:[{description:'Audit',amount:1000}]}});
