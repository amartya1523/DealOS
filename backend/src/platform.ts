...
router.post('/invitations', async (req: AuthRequest, res) => {
  ...
  const invitation = await db.$transaction(async (tx) => {
    ...
    const created = await tx.organizationInvitation.create({ data: { organizationId: organization.id, email: parsed.data.email, accessRole: parsed.data.accessRole, businessRole: parsed.data.businessRole, tokenHash, invitedById: null, platformActorId: req.auth!.platformActorId, expiresAt: new Date(Date.now() + 7 * 86400000) } });
