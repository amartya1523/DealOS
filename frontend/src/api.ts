export type Workspace = {
  ...
  organization: { id: string; name: string; rfqHandlingMode?:'LEAD_FIRST'|'DIRECT_DRAFT'; dealRoomReady?:boolean };
...
export type ProductVariant={id:string;name:string;sku:string;priceDelta:number|string;costDelta:number|string;active:boolean};
export type Product = { id:string;name:string;sku:string;category:string;description:string;unit:string;brand?:string|null;price:number|string;cost:number|string;taxRate:number|string;recurring:boolean;cadence?:string;active:boolean;storeVisible?:boolean;featured?:boolean;variants?:ProductVariant[];stocks:Array<{onHand:number;reserved:number;minAlertLevel?:number;maxCapacity?:number|null;warehouse:{name:string}}> };
export type PriceList={id:string;name:string;currency:string;customerTier?:string|null;active?:boolean;effectiveFrom?:string;effectiveTo?:string|null;items?:Array<{id:string;productId:string;variantId?:string|null;unitPrice:number|string;product:{id:string;name:string;sku:string};variant?:{id:string;name:string;sku:string}|null}>};
export type Policy = {id:string;tier:string;maxDiscount:number|string;hardwareLimit:number|string;servicesLimit:number|string;subscriptionLimit:number|string;financeThreshold:number|string;aggregateDiscountLimit:number|string;minimumMarginPercent:number|string;approvalSequence?:string[];managerReviewerId?:string|null;financeReviewerId?:string|null;version:number;publishedAt:string};
...
export type Notification={id:string;title:string;body:string;resourceType:string;resourceId:string;state:'UNREAD'|'READ';readAt?:string|null;createdAt:string};
...
export type QuotationDetail = QuotationSummary & {
  ...
  priceLists?:PriceList[];
  lines:Array<{id:string;productId:string;variantId?:string|null;quantity:number;unitPrice:string;unitCost?:string;discount:string;allowedDiscount:string;product:{id:string;name:string;sku:string;category:string;description:string;unit:string;price:string;cost?:string;taxRate:string;recurring:boolean;cadence:string|null;active:boolean;variants?:ProductVariant[]}}>;
  ...
  catalog:Array<{id:string;name:string;sku:string;category:string;description:string;unit:string;price:string;cost?:string;taxRate:string;recurring:boolean;cadence:string|null;active:boolean;variants?:ProductVariant[]}>;
  addOns?:Array<{id:string;name:string;sku:string;category:string;cadence:string;marginContribution:string;netContribution:string;score?:number;reason?:string}>;
