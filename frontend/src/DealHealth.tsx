...
function HealthAlert({ alert, quote, data, expanded, toggle, mutate, open }: { alert: Alert; quote?: Quote; data: Workspace; expanded: boolean; toggle: () => void; mutate: (path: string, body: unknown, method?: string, message?: string) => Promise<void>; open: (view: HealthView, id?: string) => void }) {
  ...
  return <article className={`health-alert ${risk.level.toLowerCase()}`}>
    ...
    <div className="health-alert-actions">
      {canNudge && <button className="button primary" disabled={alert.nudged} onClick={() => mutate(`/alerts/${alert.id}/nudge`, {}, "POST", "Representative nudged")}><BellRing/>{alert.nudged ? "Rep nudged" : "Nudge rep"}</button>}
      ...
      {alert.kind === "DISCOUNT_ANOMALY" && canNudge && <button className="button ghost" disabled={alert.nudged} onClick={() => mutate(`/alerts/${alert.id}/escalate`, {}, "POST", "Alert escalated")}><TrendingUp/>Escalate</button>}
      <button className="button quiet" onClick={() => mutate(`/alerts/${alert.id}/resolve`, {}, "POST", "Alert dismissed")}><X/>Dismiss</button>
