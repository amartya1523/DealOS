      ...
      {selected&&<section className="join-request-detail" aria-labelledby="selected-request-title">
        ...
        {selected.status==='PENDING'?<section className="request-decision" aria-labelledby="request-decision-title">
          ...
          {decisionMode==='APPROVE'?<div className="approval-workspace">
            ...
            <div className="decision-action-row"><p><ShieldCheck/>{selected.marketplaceInterest?'The request is routed only after ownership is assigned.':'The customer will be active immediately after approval.'}</p><button className="button primary" onClick={approve} disabled={busy||!teamId||!repId}>{busy?<><RefreshCw className="spin"/>{selected.marketplaceInterest?'Assigning sales team…':'Creating customer…'}</>:selected.marketplaceInterest?'Approve & assign sales team':'Approve & create customer'}</button></div>
