...
export function QuotationsPage({user,openQuote,onCreated}:{
  ...
  const canCreate = ['REP','MANAGER','ADMIN'].includes(user.role);
  ...
  return <section className="quotations-page" aria-labelledby="quotation-list-title">
    ...
    {newOpen&&<NewQuotationDialog customers={customers} role={user.role} close={()=>setNewOpen(false)} onCreated={onCreated}/>}
  </section>;
}
...
function NewQuotationDialog({customers,role,close,onCreated}:{customers:CustomerOption[];role:string;close:()=>void;onCreated:(id:string,revisionId:string|null)=>Promise<void>}) {
  ...
  const [customerId,setCustomerId]=useState(''); const [ownerId,setOwnerId]=useState(''); const selected=customers.find(customer=>customer.id===customerId); const ownerRequired=role!=='REP';
  const assignedPeople=selected?[selected.primaryRepresentative,...(selected.collaborators??[])].filter((person):person is NonNullable<typeof person>=>Boolean(person)):[];
  ...
  const submit=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();setFormError('');if(!customerId){setFormError('Select an assigned customer.');customerRef.current?.focus();return}if(ownerRequired&&!ownerId){setFormError('Select an assigned representative to own this quotation.');return}const form=new FormData(event.currentTarget);setSubmitting(true);try{const quotation=await request<QuotationSummary>('/quotations',{method:'POST',body:JSON.stringify({customerId,...(ownerRequired?{ownerId}:{}),validUntil:toIsoDate(String(form.get('validUntil')??'')),promisedDeliveryAt:toIsoDate(String(form.get('promisedDeliveryAt')??'')),terms:String(form.get('terms')??'').trim()||undefined})});close();await onCreated(quotation.id,quotation.currentRevisionId)}catch(error){setFormError(error instanceof ApiError?error.message:error instanceof Error?error.message:'Unable to create quotation.')}finally{setSubmitting(false)}};
  const backdropKey=(event:ReactKeyboardEvent<HTMLDivElement>)=>{if(event.key==='Escape'&&!submitting)close()};
  return <div className="quotation-dialog-backdrop" onMouseDown={event=>event.target===event.currentTarget&&!submitting&&close()} onKeyDown={backdropKey}><div className="quotation-dialog" role="dialog" aria-modal="true" aria-labelledby="new-quotation-title" aria-describedby="new-quotation-help" ref={dialogRef}><div className="quotation-dialog-head"><div><h2 id="new-quotation-title">New quotation</h2><p id="new-quotation-help">Owner, account team, tier, and currency are validated and snapshotted by the server.</p></div><button type="button" onClick={close} disabled={submitting} aria-label="Close new quotation dialog"><X/></button></div><form onSubmit={submit} noValidate><label>Customer<select ref={customerRef} value={customerId} onChange={event=>{const id=event.target.value;setCustomerId(id);const customer=customers.find(item=>item.id===id);setOwnerId(ownerRequired?customer?.primaryRepresentative?.id??'':'')}} required aria-invalid={Boolean(formError&&!customerId)}><option value="">Select an assigned customer</option>{customers.map(customer=><option key={customer.id} value={customer.id} disabled={!customer.primaryTeam||!customer.primaryRepresentative}>{customer.name} · {customer.tier}{!customer.primaryRepresentative?' · Unassigned':''}</option>)}</select></label>{selected&&selected.primaryTeam&&selected.primaryRepresentative?<><div className="customer-context"><span>Tier <b>{selected.tier}</b></span><span>Currency <b>{selected.currency}</b></span><span>Team <b>{selected.primaryTeam.name}</b></span><span>Primary rep <b>{selected.primaryRepresentative.name}</b></span></div>{ownerRequired&&<label>Deal owner<select value={ownerId} onChange={event=>setOwnerId(event.target.value)} required><option value="">Select an assigned representative</option>{assignedPeople.map(person=><option key={person.id} value={person.id}>{person.name}{person.id===selected.primaryRepresentative?.id?' · Primary':' · Collaborator'}</option>)}</select></label>}</>:selected&&<p className="dialog-error" role="alert">This customer is unassigned and cannot be used for a new quotation.</p>}<div className="quotation-date-grid"><label>Valid until <span>Optional</span><input type="date" name="validUntil" min={new Date(Date.now()+86400000).toISOString().slice(0,10)}/></label><label>Promised delivery <span>Optional</span><input type="date" name="promisedDeliveryAt"/></label></div><label>Commercial terms <span>Optional</span><textarea name="terms" maxLength={5000} placeholder="Payment, delivery or validity notes"/></label>{!customers.length&&<p className="dialog-hint">No assigned active customers are available. Ask a manager to configure an account relationship.</p>}{formError&&<p className="dialog-error" role="alert">{formError}</p>}<div className="quotation-dialog-actions"><button type="button" className="button ghost" onClick={close} disabled={submitting}>Cancel</button><button type="submit" className="button primary" disabled={submitting||!selected?.primaryTeam||!selected.primaryRepresentative||(ownerRequired&&!ownerId)}>{submitting?<><RefreshCw className="spin"/>Creating…</>:<>Create draft<ChevronRight/></>}</button></div></form></div></div>;
}
