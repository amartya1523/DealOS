...
const customerResponse={items:[{id:'customer-1',name:'Acme Corp',tier:'Gold',currency:'INR'}]};
...
describe('quotation list',()=>{
  ...
  it('applies create-action RBAC and disables writes in View As mode',async()=>{
    const {unmount}=render(<QuotationsPage user={user('MANAGER')} openQuote={vi.fn()} onCreated={vi.fn()}/>);
    await screen.findByRole('button',{name:'Open Q-1001 for Acme Corp'}); expect(screen.queryByRole('button',{name:'New quotation'})).not.toBeInTheDocument(); unmount();
