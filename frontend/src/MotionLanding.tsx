...
export function Landing() {
  const root = useRef<HTMLDivElement>(null); const [menu, setMenu] = useState(false); const [paused, setPaused] = useState(false); const [active, setActive] = useState(0); const [listening, setListening] = useState(false);
  useLayoutEffect(() => {
    ...
    mm.add({ motion: "(prefers-reduced-motion: no-preference)", desktop: "(min-width: 900px)" }, context => {
      ...
      const ctx = gsap.context(() => {
        ...
        gsap.to(".mobile-product-art", { yPercent: -13, rotate: 1.2, ease: "none", scrollTrigger: { trigger: ".continuity-section", start: "top bottom", end: "bottom top", scrub: 1.1 } });
        gsap.to(".sync-orbit", { rotate: 135, scale: 1.08, ease: "none", scrollTrigger: { trigger: ".continuity-section", start: "top bottom", end: "bottom top", scrub: 1.3 } });
        gsap.to(".voice-disc", { yPercent: -18, rotate: -8, ease: "none", scrollTrigger: { trigger: ".voice-section", start: "top bottom", end: "bottom top", scrub: 1.2 } });
        gsap.utils.toArray<HTMLElement>(".continuity-note").forEach((note, index) => gsap.from(note, { y: 45 + index * 15, opacity: 0, duration: .8, ease: "power3.out", scrollTrigger: { trigger: note, start: "top 88%", once: true } }));
